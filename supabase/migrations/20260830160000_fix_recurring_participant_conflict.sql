-- Recurring generation could not add anybody.
--
-- event_participants lost its plain unique (event_id, user_id) when guests
-- arrived: 20260628100000 replaced it with a partial index, unique only where
-- user_id is not null, because a guest row has no user at all. Postgres will
-- not infer a partial index from a bare conflict target, so every
-- `on conflict (event_id, user_id)` against that table raises rather than
-- doing nothing, and the whole generator transaction dies with it.
--
-- The fix is to name the predicate, which is what makes the index inferable.
-- Only the event_participants inserts are affected; event_member_responses
-- still has an ordinary unique constraint and its conflict targets are fine.
