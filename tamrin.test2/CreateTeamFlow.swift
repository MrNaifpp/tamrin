...
struct CreateTeamFlow: View {
    ...
    init(store: TamrinStore, isPresented: Binding<Bool>) {
        self.store = store
        _isPresented = isPresented
        #if DEBUG
        let arguments = ProcessInfo.processInfo.arguments
        if arguments.contains("-TamrinScheduleFlow") || arguments.contains("-TamrinDaysFlow") {
            var preview = TeamDraft()
            preview.teamName = "رفاق الملعب"
            _draft = State(initialValue: preview)
            _step = State(initialValue: .templates)
            _isComposing = State(initialValue: true)
            _composerInitialSheet = State(initialValue: .schedule)
        } else if arguments.contains("-TamrinComposerFlow") || arguments.contains("-TamrinCapacityFlow") || arguments.contains("-TamrinTemplatesFlow") || arguments.contains("-TamrinMembersFlow") {
            var preview = TeamDraft()
            preview.teamName = "رفاق الملعب"
            var plan = PlanDraft()
            plan.name = "كورة الثلاثاء"
            plan.weekdays = [3, 5]
            plan.locationName = "ملعب الحي الرياضي"
            plan.locationAddress = "طريق الأمير تركي، الرياض"
            plan.price = 35
            let composing = arguments.contains("-TamrinComposerFlow") || arguments.contains("-TamrinCapacityFlow")
            if !composing { preview.plans = [plan] }
            _draft = State(initialValue: preview)
            _step = State(initialValue: arguments.contains("-TamrinMembersFlow") ? .members : .templates)
            _isComposing = State(initialValue: composing)
            if composing { _composerPlan = State(initialValue: plan) }
            if arguments.contains("-TamrinCapacityFlow") { _composerInitialSheet = State(initialValue: .capacity) }
        }
        #endif
    }
    ...
    private func create() {
        createdTeam = store.createTeam(from: draft)
        UINotificationFeedbackGenerator().notificationOccurred(.success)
    }
...
private struct ScheduleSheet: View {
    ...
    private func scheduleKindButton(_ kind: ScheduleKind, title: String, symbol: String) -> some View {
        Button {
...
private struct PriceSheet: View {
    ...
    private let quickAmounts: [Double] = [0, 20, 35, 50]
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Picker("النوع", selection: $method.kind) {
                        Text("حساب بنكي").tag(PaymentKind.bank)
                        Text("نقداً").tag(PaymentKind.cash)
...
private struct CreationSuccessPage: View {
    ...
    var body: some View {
        VStack(spacing: 22) {
            ...
            VStack(spacing: 10) {
                ShareLink(
                    item: URL(string: "tamrin://join/\(team.inviteCode)")!,
...
struct CreateExerciseFlow: View {
    @Bindable var store: TamrinStore
    @Environment(\.dismiss) private var dismiss
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                TamrinTheme.page.ignoresSafeArea()
                TemplateComposerPage(plan: $plan, paymentMethods: $paymentMethods) {
                    store.createPlan(from: plan)
                    UINotificationFeedbackGenerator().notificationOccurred(.success)
                    dismiss()
                } cancel: { dismiss() }
            }
            .navigationTitle("تمرين جديد")
        ...
        .environment(\.layoutDirection, .rightToLeft)
        .interactiveDismissDisabled(!plan.name.isEmpty)
    }
}
